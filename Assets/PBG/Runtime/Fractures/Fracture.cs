...
namespace PBG.Runtime.Fractures
{
    public static class Fracture
    {
        public static ChunkGraphManager FractureGameObject(GameObject gameObject, Anchor anchor, int seed,
            int totalChunks, Material insideMaterial, Material outsideMaterial, float jointBreakForce, float density)
        {
            ...
            foreach (var chunk in chunks) ConnectTouchingChunks(chunk, jointBreakForce);

            // Set anchored chunks as kinematic
            AnchorChunks(gameObject, anchor);

            var fractureGameObject = new GameObject("Fracture");
            foreach (var chunk in chunks) chunk.transform.SetParent(fractureGameObject.transform, false);
        ...
        private static void AnchorChunks(GameObject gameObject, Anchor anchor)
        {
            ...
            foreach (var collider in anchoredColliders)
            {
                ...
                if (chunkRb) chunkRb.isKinematic = true;
            }
        }

        private static List<GameObject> BuildChunks(Material insideMaterial, Material outsideMaterial,
            List<Mesh> meshes, float chunkMass)
        {
        ...
        private static List<Mesh> FractureMeshesInNvblast(int totalChunks, NvMesh nvMesh)
        {
            ...
            for (var i = 1; i < meshCount; i++) meshes.Add(ExtractChunkMesh(fractureTool, i));
        ...
        private static void ConnectTouchingChunks(GameObject chunk, float jointBreakForce, float touchRadius = .01f)
        {
            ...
            for (var i = 0; i < vertices.Length; i++)
            {
                ...
                for (var j = 0; j < hits.Length; j++) overlaps.Add(hits[j].GetComponent<Rigidbody>());
            }
